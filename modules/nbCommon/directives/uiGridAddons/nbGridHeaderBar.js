(function() {
    ...
    function NbGridHeaderBarDirective($sce, $filter, $timeout) {
        ...
        function compile() {
            return {
                pre: function(scope) { // , ele, attr
                    ...
                    scope.searchIcon = options.headerSearchIcon || '<i class="icon_nb_search"></i>';
                    scope.closeIcon = options.headerSearchCloseIcon;
                    scope.searchPlaceHolder = options.headerSearchPlaceHolder || 'Search...';
        ...
        function Controller($scope) {
            ...
            self.totalCount = self.gridApi.grid.rows.length;

            // auto refresh items count when rows re-rendered
            self.gridApi.grid.api.core.on.rowsRendered(self.gridApi.grid.appScope, function() {
                self.totalCount = self.gridApi.grid.rows.length;
            });
