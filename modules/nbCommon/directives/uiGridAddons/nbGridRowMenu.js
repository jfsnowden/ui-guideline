            ...
            return {
                pre: function (scope, ele, attrs, uiGridCtrl) {
                    ...
                    if (isEnabledRowMenu) {
                        ...
                        scope.callActionMenuDropdown = function callActionMenuDropdown(item, event) {
                    ...
                    function open(event, actionMenuEle) {
                        ...
                        if (totalHeight > docHeight) {
                            top = top - (totalHeight - docHeight + 15);
                        }
                    ...
                    function appendActionMenuDropdown(gridId, scope) {
                        scope.rowActionMenuList = [];
                        ...
                        uiGridCtrl.grid.api.core.on.rowsRendered(uiGridCtrl.grid.appScope, function () {
                            $timeout(function () {
                                if ($(ele).is(':visible')) {
                                    ...
                                    totalHeight = rowHeight * rows.length + emptyCanvasHeight;

                                    menuBtnOffset = 46;
                                    var isGroupHeaderActionMenu = uiGridCtrl.grid.options.enableGroupHeaderActionMenu;
                                    angular.forEach(rows, function (row) {
                                        if ($(row).parents(".ui-grid-pinned-container").length > 0 || isGroupHeaderActionMenu != undefined && !isGroupHeaderActionMenu && $(row).hasClass('ui-grid-tree-header-row')) {
                                            return;
                                        }
