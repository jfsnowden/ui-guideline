...
(function() {
    ...
    angular.module('nb.common')
        ...
        .filter('nbHighlightEx', [
            '$sce', 'nb.ng.utilitySrvc',
            function($sce, util) {
                return function(text, phrase, ipsInSubnet) {
                    if (!text || !phrase) return undefined;
                    ...
                    function isHighlight(filterStr, text2) {
                        var lowerFilterStr = filterStr.toLowerCase();
                        if (text2.toLowerCase().indexOf(lowerFilterStr) >= 0) return true;
                        var notHighlightArray = ['mgmt', 'vendor', 'model'];
                        for (var i = 0; i < notHighlightArray.length; i++) {
                            if (lowerFilterStr.indexOf(notHighlightArray[i]) >= 0) return false;
                        }
                    ...
                    _.each(subText, function(text3, index) {
                        if (ipsInSubnet && ipsInSubnet.length > 0 && util.isIpOrIpMask(text3)) {
                            var ip = text3.split('/')[0];
                        ...
                        text3 = text3.replace(/\$class/gi, 'filter-highlight');
                        subText[index] = text3;
                    });
        ...
        .filter('parseDate', [
            function() {
                return function(strValue) {
                    return strValue ?
                        (
                            angular.isDate(strValue) ? strValue : new Date(strValue)
                        ) :
                        strValue;
                };
            }
        ...
        .filter('personListFilter', function () {
            return function (personList, arg) {
                var tempPersonList = [];
                if (personList.length > 0) {
                    for (var i = 0; i < personList.length; i++) {
                        if (personList[i].name.toLowerCase().indexOf(arg.toLowerCase()) >= 0 || personList[i].email.toLowerCase().indexOf(arg.toLowerCase()) >= 0) {
                            tempPersonList.push(personList[i]);
                        }
                    }
                }
